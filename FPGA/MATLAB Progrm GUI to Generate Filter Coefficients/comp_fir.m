...
function cic_filter_with_sliders
    ...
    function updatePlot(~, ~)
      ...
      % Display for additional calculated values
        ...
        % Create a Direct Form FIR object with your coefficients
        B = 16;
        hz = h/max(h);
        hz = round(hz*power(2,B-1)-1);
        hd = dfilt.dffir(hz);
        
        % Set arithmetic to fixed-point
        set(hd, 'arithmetic', 'fixed');
        
        % Specify the full file path
        filepath = 'my_filter.coe';

        % Write the coefficients to the specified path
        radix = 16;             % Use hexadecimal representation
        coewrite(hd, radix, filepath);        % Write the coefficients to a .coe file
    end
end
